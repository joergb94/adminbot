...
import ShowDataBot from './ShowDataBot.vue';
...
const titleDetail = computed(() => getters[`${moduleName}/getDetailTitle`]);
...
const showModalFormDetail = computed({
    get: () => getters[`${moduleName}/getShowModalFormDetail`],
    set: (show) => dispatch(`${moduleName}/showModalFormDetailState`, show),
});
...
const showModalFormHandlerDetail = (show) => {
    showModalFormDetail.value = show;
};
onMounted(() => {
...
<template>
        ...
        <Card class="col-12 neo-background-white">
            ...
            <Dialog v-model:visible="showModalFormDetail" modal :style="{ width: '85rem' }">
                <template #header>
                    <div class="text-2xl font-bold" v-text="titleDetail"></div>
                </template>
                <ShowDataBot/>
